...
def print_statistics(args: argparse.Namespace,
    ...
    if args.full_stats:
        ...
        if strings:
            ...
            shortest_line = min(strings, key=len)
            longest_line = max(strings, key=len)
            min_length = len(min(strings, key=len))
            max_length = len(max(strings, key=len))
            print(f"   shortest line: '{shortest_line}', length is {min_length}")
            print(f"   longest line: '{longest_line}', length is {max_length}")
